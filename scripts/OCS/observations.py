...
processing/analysis functions for OCS observations
...
def get_OCS_obs(target='OCS', limit_depth_to=20,):
    """
...
#    df = pd.read_csv(folder+filename)
    df = get_SL_sea_water_database_OCS()
...
def get_SL_sea_water_database_OCS(target='OCS'):
    """

    """
#    filename = 'Lennartzetal_CS2_seawater.csv'
    filename = 'Lennartzetal_OCS_seawater.csv'
    data_root = utils.get_file_locations('data_root')
    folder = '{}/{}/inputs/'.format(data_root, target)
...
def get_SL_sea_water_database_metadata(target='OCS'):
    """
...
def process_obs_and_ancillaries_2_csv(target='OCS',
...
def get_processed_df_obs_mod(reprocess_params=False, target='OCS',
                             filename='s2s_OCS_obs_ancillaries.csv',
...
def add_extra_vars_rm_some_data(df=None, target='OCS',
