...
processing/analysis functions for CS2 observations
...
def get_CS2_obs(target='CS2', limit_depth_to=20,):
    """
    Get the raw observations from Sinikka Lennartz's database
    """
    # File to use
#    filename = 'SL_seawater_concs_above_{}m.csv'.format(limit_depth_to)
    # Where is the file?
#    data_root = utils.get_file_locations('data_root')
#    folder = '{}/{}/inputs/'.format(data_root, target)
#    df = pd.read_csv(folder+filename)
    df = get_SL_sea_water_database_CS2()
    # Variable name? - Just use one of the values for now
    Varname = 'OCS conc [pmol/L]'
...
#        except:
        try:
            ...
            return np.NaN
    # Add datetime
    # NOTE: This will include 346 NaNs for CS2
...
def get_SL_sea_water_database_CS2(target='CS2'):
    """

    """
#    filename = 'Lennartzetal_CS2_seawater.csv'
    filename = 'Lennartzetal_CS2_seawater.csv'
    data_root = utils.get_file_locations('data_root')
    folder = '{}/{}/inputs/'.format(data_root, target)
    df = pd.read_csv(folder+filename, skiprows=5, delimiter=',')
    return df



def get_SL_sea_water_database_metadata(target='CS2'):
    """

    """
    filename = 'Lennartzetal_flags.txt'





def process_obs_and_ancillaries_2_csv(target='CS2',
...
def get_processed_df_obs_mod(reprocess_params=False, target='CS2',
                             filename='s2s_CS2_obs_ancillaries.csv',
...
def add_extra_vars_rm_some_data(df=None, target='CS2',
